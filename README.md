...
8. ✅ **跨进程嵌入 Qt** — `CrossProcessChart` 将 pywebview 图表窗口通过 HWND 嵌入到 PySide6/PyQt6 QWidget，支持无边框、窗口大小同步，类似 Chrome 多进程架构（仅 Windows）
...
19. **跨进程嵌入 Qt** — `CrossProcessChart` 通过 HWND 将 pywebview 窗口嵌入 QWidget（仅 Windows）。
