...
**新增:**
1. ✅ **ReflexChart** — 全新的 `ReflexChart(StaticLWC)` 类，让你在 [Reflex](https://reflex.dev) Web 应用中嵌入 K 线图
2. ✅ **示例 27** — 完整的 Reflex 示例（含 SMA 指标、暗色主题）
...
`ReflexChart` 支持两种模式：
- **纯 HTML 生成**（无需安装 reflex）— `chart.get_html()`
- **Reflex 组件嵌入** — `chart.to_reflex()` 返回 `rx.Component`，直接放入你的 Reflex 页面
...
**主要支持环境：** PySide6、wxPython、asyncio、Reflex。
