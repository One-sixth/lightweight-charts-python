"""ind_sys 最小示例 — 1 Chart × 2 pane（candle + volume），同步线程动态更新
...
if __name__ == '__main__':
    ...
    def live_feed():
        ...
        for n in range(1, 241):
            ...
            o, h, l, c_ = nc - np.random.rand(), nc + np.random.rand(), \
                           nc - np.random.rand(), nc

            # 只更新数据层，同步线程自动检测变化并推送渲染层
            sys_obj['candle'].append(pd.DataFrame({
                'time': [t], 'open': [o], 'high': [h], 'low': [l], 'close': [c_],
            }))
            sys_obj['volume'].append(pd.DataFrame({
