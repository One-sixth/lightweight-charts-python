...
| live 动态同步（互斥锁保护） | ✅ 完成 |
| 冒烟测试 | ✅ 10+ 项全过 |
| 示例 1 — minimal | ✅ `examples/1_minimal/minimal.py`（1Chart/2Pane/candle+vol/240次0.25s更新） |
...
├── examples/
│   ├── 1_minimal/
│   │   └── minimal.py   # 最小示例：1 Chart × candle + volume，同步线程动态更新
│   └── 2_demo/
│       └── demo.py      # 复杂示例：2Window/5Chart/22Series/4pane/EMA品种/30次随机更新
└── tests/
    └── smoke_test.py    # 冒烟测试
```
...
sys_obj['candle'].append(df_new)             # 追加（replace-or-append：仅替换最后一条）
...
**触发**：`sys_obj.build(live=True)` 启动守护线程（检测频率 0.1 秒）
...
*ind_sys 子系统记忆，泰斗先生 × 小音，2026-07-04*
