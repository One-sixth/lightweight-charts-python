...
| 文件 | 变更 |
|------|------|
| `lightweight_charts/abstract.py` | 新增 `_sync_interval_to_js()` 方法，在 `set_period()` 和 `set()` 中调用，将 `self._interval` 同步到 JS 端并动态设置 `secondsVisible` |
| `lightweight_charts/js/bundle.js` | `timeFormatter` 新增秒级分支，`_interval < 60` 时显示 `HH:MM:SS` |

#### 时间格式一览

| 时间级别 | 间隔 | 轴刻度 | 标签格式示例 |
|---------|------|--------|-------------|
| 日线+ | `>= 86400s` | 关闭秒 | `2026-07-08` |
| 分钟级 | `60~86399s` | 关闭秒 | `2026-07-08 14:30` |
| 秒级 | `< 60s` | 开启秒 | `2026-07-08 14:30:45` |
