...
**chart_model** 是 lightweight-charts-onesixth 的纯数据指标系统子包（`chart_model/`）。它提供声明式、扁平化、引用式的数据模型，与渲染层完全解耦。
...
### ⚠️ 同步线程是唯一的渲染通道
- **不允许直连渲染层**——所有数据更新必须通过 `model['name'].set/append/pop`
...
| `examples/01_hello_world/minimal.py` | 最小示例 |
| `examples/02_multi_window_dashboard/demo.py` | 复杂示例 |
...
5. **冒烟测试** — 修改后必须跑 `python -m chart_model.tests.smoke_test`
