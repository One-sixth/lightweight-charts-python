...
| `update_batch()` | 原有实现 | 抽取 `_clean_update_batch()` 辅助方法 | ✅ 改善代码复用 |
...
| `update_batch()` | Candlestick.update_bars() | 委托到 `self.candle.update_batch()` | ✅ |
...
| `chart.update_batch(df)` | 直接调用 | 委托到 candle.update_batch() | ✅ 完全兼容 |
...
| 功能模块 | 状态 | 说明 |
|---------|------|------|
| **K 线设置/更新** | ✅ 完整 | set/update/update_batch 数据流完整 |
...
在 `VolumeSeries.update()` 和 `update_batch()` 中也有类似的涨跌颜色逻辑。确认两处逻辑一致（从代码中看确实一致）。
